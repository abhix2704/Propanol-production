...
resources = np.array(df["Resources"])
...
for i in range(0,len(resources)):
    if allCategories[i] == 'Natural gas':
        naturalGas.append(resources[i])
    elif allCategories[i] == 'Carbon dioxide':
        carbonDioxide.append(resources[i])
    elif allCategories[i] == 'Biogas':
        biogas.append(resources[i])
    elif allCategories[i] == 'Oxygen':
        oxygen.append(resources[i])
    elif allCategories[i] == 'Hydrogen':
        hydrogen.append(resources[i])
    elif allCategories[i] == 'Ethylene':
        ethylene.append(resources[i])
    elif allCategories[i] == 'Utilities + fixed costs':
        utilities.append(resources[i])
    elif allCategories[i] == 'CAPEX':
        capex.append(resources[i])
    elif allCategories[i] == 'BAU':
        bau.append(resources[i])
...
x1 = cumData["Resources"]
y1 = cumData['yCoordinate']
plt.scatter(x1, y1, s = 25, marker = 'D', color = 'black')
plt.legend(loc = "right")
plt.xlabel("Resources [USD$_\mathrm{2013}$ kg$^\mathrm{-1}$ propanol]")
