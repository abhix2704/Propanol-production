...
humanHealth = np.array(df["Human health"])
...
capex = []
bau = []
for i in range(0,len(humanHealth)):
    if allCategories[i] == 'Natural gas':
        naturalGas.append(humanHealth[i])
    elif allCategories[i] == 'Carbon dioxide':
        carbonDioxide.append(humanHealth[i])
    elif allCategories[i] == 'Biogas':
        biogas.append(humanHealth[i])
    elif allCategories[i] == 'Oxygen':
        oxygen.append(humanHealth[i])
    elif allCategories[i] == 'Hydrogen':
        hydrogen.append(humanHealth[i])
    elif allCategories[i] == 'Ethylene':
        ethylene.append(humanHealth[i])
    elif allCategories[i] == 'Utilities + fixed costs':
        utilities.append(humanHealth[i])
    elif allCategories[i] == 'CAPEX':
        capex.append(humanHealth[i])
    elif allCategories[i] == 'BAU':
        bau.append(humanHealth[i])
...
x1 = cumData["Human health"]
y1 = cumData['yCoordinate']
plt.scatter(x1, y1, s = 25, marker = 'D', color = 'black')
plt.legend(loc = "right")
plt.xlabel("Human health [DALY kg$^\mathrm{-1}$ propanol]")
